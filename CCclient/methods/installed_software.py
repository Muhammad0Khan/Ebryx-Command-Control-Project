...
def get_installed_software(token):
    try:
        ...
        result = subprocess.run(['system_profiler', '-json', 'SPApplicationsDataType'], capture_output=True, text=True)
        ...
        modified_applications = [app for app in applications if parse_datetime(app.get('lastModified')) >= five_days_ago]
